...
jump and get your vertical in centimeters, along with the takeoff and landing frames the measurement came from.

Height is calculated from time in the air (`h = g·t²/8`) rather than from pixels, so no reference
object or calibration is needed in the shot — just two clean frames. Pose estimation runs on-device with MediaPipe, so your video never leaves your device.
